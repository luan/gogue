...
var _ = Describe("Map", func() {
  Describe("NewMap", func() {
    ...
    It("errors if there is no player", func() {
      _, err := NewMap(`*`)
      Expect(err.Error()).To(Equal("Map requires a Player(@)"))
    })
    ...
    It("knows where the player is", func() {
      newMap, _ := NewMap(`.@..*`)
      Expect(newMap.Player.X).To(Equal(1))
      Expect(newMap.Player.Y).To(Equal(0))
    })
